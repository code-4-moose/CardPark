    ...
    public class UserService : IUserService
    {
        ...
        public User GetSavedStudent()
        {
            ...
            using (IDbConnection con = new SqlConnection(Global.ConnectionString))
            {
                ...
                var Id = 9;
                var oStudents = con.Query<User>("SELECT * FROM Students WHERE StudentId = @StudentId", new{ Id }).ToList();

                if (oStudents != null && oStudents.Count > 0)
                {
                    _oUser = oStudents.FirstOrDefault();
                }
            }
            return _oUser;
        }
        ...
        public User Save(User oUser)
        {
            ...
            using (IDbConnection con = new SqlConnection(Global.ConnectionString))
            {
                ...
                var oUser = con.Query<User>("SP_User",
